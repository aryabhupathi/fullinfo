...
// export default FeePayment;
  ...
  const FeePayment = () => {
    ...
    //   if (name === "activity") {
    //     try {
    //       const res = await fetch(`${ACTIVITY_URL}/${encodeURIComponent(value)}`);
    //       console.log(res, "rrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrr")
    //       if (res.ok) {
    //         const data = await res.json();
    //         setFormData((prev) => ({
    //           ...prev,
    //           totalFee: data.fee || "",
    //           dueDate: formatDate(data.date),
    //         }));
    //       }
    //     } catch (err) {
    //       console.error("Activity lookup failed:", err);
  ...
  if (name === "activity") {
    const selectedActivity = activity.find(
      (act) => act.activityName === value
    );
    if (selectedActivity) {
      setFormData((prev) => ({
        ...prev,
        totalFee: selectedActivity.fee || "",
        dueDate: formatDate(selectedActivity.date),
      }));
    }
  }
};
            ...
            <Form onSubmit={handleSubmit}>
              ...
              <Row>
                <Col md={3} className="mb-3">
                  ...
                  <Form.Select
  name="activity"
  value={formData.activity} // Should be activity._id or activityName
  onChange={handleChange}
  required
>
  <option value="">Select Activity</option>
  {activity.map((act) => (
    <option key={act._id || act.activityName} value={act.activityName}>
      {act.activityName}
    </option>
  ))}
</Form.Select>
